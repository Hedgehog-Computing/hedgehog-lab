...
class Mat {
    ...
    clear() { this.val = []; this.rows = 0; this.cols = 0; this.digits = -1; this.mode = ''; return this; }
    ...
    Ns(row: number, col: number, N: number): Mat {
        ...
        for (let i = 0; i < row; i++) { this.val.push(Array(col).fill(N)); }
        return this;
    }
    ...
    T(): Mat { // transpose
        ...
        for (let row = 0; row < this.rows; row++) { for (let col = 0; col < this.cols; col++) returnMatrix.val[col][row] = this.val[row][col]; }
        return returnMatrix;
    }
    ...
    minusScalar(val: number): Mat { return this.addScalar(val * (-1)); }
    divideScalar(val: number): Mat { return this.multiplyScalar(1.0 / val); }
    ...
    multiply(rightMat: Mat): Mat {  return multiply(this, rightMat); }
    dotMultiply(rightMat:Mat):Mat { return dotMultiply(this, rightMat);}
    divide(rightMat:Mat):Mat {return divide(this, rightMat);}
    ...
    col(colIndex: number): Mat {
        this.dimCheck(0, colIndex); let columnVector = Array(this.rows).fill(0);
    ...
    reshape(row: number, col: number): Mat {
        let returnMatrix = new Mat().zeros(row, col); let thisArray = this.toArray(); let arrayPt = 0;
        for (let i = 0; i < row; i++) {
            for (let j = 0; j < col; j++) {
    ...
    Row(row:number): Mat{ return new Mat(this.RowVector(row));}

    //get a column of matrix in N-by-1 matrix format
    Col(col:number): Mat{ return new Mat(this.ColVector(col));}
...
function multiply(leftMat: Mat, rightMat: Mat): Mat {
    ...
    let m = leftMat.rows, n = leftMat.cols, p = rightMat.cols;
