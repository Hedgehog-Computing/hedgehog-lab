...
Gives the `k` minimum elements  from the matrix in the form of a sub-matrix.
...
**returns:** `any[] | Mat` - a submatrix (`Array` or `Mat`) 

---

## Description

If 'A' is a matrix, it will return a matrix whose columns or rows contain the k smallest elements of each columns or rows of A (based on the parameter `dim`), returned as a `Mat`. If it is a list, it'll return a list containing the k smallest elements of that list.(or list for the list case). And for a matrix, you have to provide it a dimension (default is `dim = 1`, column), it will return the `k` smallest elements in each column of A, as a new matrix. It's similar to `min()` where it returns the smallest values, but can returns more now, specified by the parameter `k`. 
