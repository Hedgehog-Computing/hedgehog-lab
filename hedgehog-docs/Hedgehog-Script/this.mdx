...
const obj = {a: 'Custom' };
//variables declared with var become properties of the global objects (since var is global)
var a = 'Global';

function whatIsThis() {
    return this.a; //the value of this return statement is dependent on how the function is called as well see below
}

WhatIsThis(); // 'Global' as this in the function isn't set so it default to the global/window object
WhatIsThis.call(obj) // 'Custom' - the functions `this` is bound to call's first parameter: a: 'Custom', so returning this.a returns 'Custom'
WhatIsThis.apply(obj) // 'Custom' - works similar to call()
```
