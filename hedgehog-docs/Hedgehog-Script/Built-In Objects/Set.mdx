...
The `Set` is an object available as a data structure. 

- It has similarities and differences relative to `Maps` and `Objects`. 

    - `Maps` resemble `Sets` the most.
...
<MathJaxContext>
<MathJax>
<p> That is, just like maps, internally they can be like a hash table with <span>{ "\\(\\Omega(1)\\)" }</span> lookup time.</p>

<p>Then a search at <span>{ "\\(\\Omega(\\log(N))\\)" }</span> time, and anything else better than <span>{ "\\(\\Omega(N)\\)" }</span> time. </p>
</MathJax>
</MathJaxContext>
:::
...
Lastly, a practical function of `Sets` is to decompose `Strings`:
