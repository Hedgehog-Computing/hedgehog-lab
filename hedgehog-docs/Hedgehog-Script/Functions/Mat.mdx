...
- The `Mat` Object is very useful for matrix-like operations in Hedgehog Script. 
  - It can be 1 dimensional, 2d, 3d and so on. 
...
All built in functions for Hedgehog Lab work for `Arrays`, `Tensor` objects and `Mat` objects. 

However, if you only have a normal `Array` you are limited to Hedgehog Script `Array` libraries. 
...
Check out the "Math" book to become more familiar. 
