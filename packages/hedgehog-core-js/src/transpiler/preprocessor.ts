...
async function parseRegisterdPackageWithoutPackageJsonFile(
  ...
  if (splittedResult.length != 2)
    throw "Invalid importing library: " + secondPart;
  ...
  //get package location
  const packageLocation = getPackageLocation(
    packageName.replace(/\s/g, ""),
    theFullListInJson
  );
    ...
    for (let i = 0; i < vecSplittedString.length; i++) {
      ...
      //    and fetch the second part recursively (which should be and must be a valid URL or a registered package)
        ...
        if (containsURL(splittedResult[1])) {
          ...
          const libraryFromUrl = await fetch(splittedResult[1], {
            method: "get",
          }).then((body) => {
          ...
          const strCallStack =
            strCurrentCallStack +
            " -> " +
            splittedURLResult[splittedResult.length - 1];

          //3.2.4 process the big chunk of code
          const currentResult = await preprocessDFS(
            libraryFromUrl,
            strCallStack
          );
